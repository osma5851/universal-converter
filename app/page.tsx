...
import { getCategoryName, getUnitName } from '@/lib/translations';
  ...
  return (
    <main className="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <header className="text-center mb-12 animate-fade-in pt-8">
        ...
        <section className="mt-12 animate-fade-in" style={{ animationDelay: '0.2s' }}>
          <div className="bg-white/40 backdrop-blur-sm rounded-2xl p-6 border border-cream-200">
            <h3 className={`text-lg font-semibold text-slate-700 mb-4 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <ArrowLeftRight size={20} className="text-coral-500" />
              {t('quickReference')} {getCategoryName(language, selectedCategory.id)}
            </h3>
